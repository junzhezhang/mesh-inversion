...
import numpy as np
from lib.rendering.cmr_meshzoo import iso_sphere
...
def compute_uvsampler(verts, faces, tex_size=2):
    ...
    coords = np.stack([p for p in itertools.product(*[alpha, beta])]) # (36, 2)
    vs = verts[faces] # (656, 3, 3) (F, 3 nodes, posistions)
