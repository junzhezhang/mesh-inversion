...
from lib.rendering.utils import circpad, symmetrize_texture, adjust_poles
...
class MeshDiscriminator(nn.Module):
    ...
    def forward(self, texture, mesh_map, c=None, caption=None, others=None):
        ...
        elif self.args.conditional_encoding:
            feat = others['encoding']
            emb = self.emb(feat)
            y += torch.sum(x * emb.unsqueeze(-1).unsqueeze(-1), dim=1, keepdim=True)
...
class TextureDiscriminator(nn.Module):
    ...
    def forward(self, x, c=None, caption=None, others=None):
...
class MultiScaleDiscriminator(nn.Module):
    ...
    def forward(self, x, mesh_map=None, c=None, caption=None, others=None):
        d1, m1 = self.d1(x, c, caption, others)
        d2, m2 = self.d2(x, mesh_map, c, caption, others)
        if self.args.num_discriminators == 3:
            d3, m3 = self.d3(x, c, caption, others)
...
class Generator(nn.Module):
    def __init__(self, args, emb_dim, symmetric=True, mesh_head=True):
        ...
        elif args.conditional_encoding:
            self.emb = nn.Sequential(
        ...
        self.param_list+=[self.blk4, self.blk5, self.blk6, self.conv_final]
        ...
        if mesh_head:
            ...
            self.param_list+=[self.blk3_mesh, self.conv_mesh]
    ...
    def forward(self, z, c=None, caption=None, others=None, return_attention=False):
        if self.args.conditional_class:
        ...
        if self.args.conditional_encoding:
            feat = others['encoding']
            emb = self.emb(feat)
            z = torch.cat((z, emb), dim=1) # Concatenate random vector and encoding
        ...
        # ([64, 3, 512, 512]) x_tex 
        # torch.Size([64, 3, 32, 32]) x_mesh
        # torch.Size([64, 25, 16, 16])  attention_map  
        if return_attention: # default is true
    ...
    def get_params(self):
        ...
        return [{'params': itm.parameters()} for itm in self.param_list]
        # {'param'}
