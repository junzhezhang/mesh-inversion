...
class ShapeRetriever(nn.Module):
    """
    output classification
    """
    def __init__(self, nz_feat, n_mean_shapes):
        super(ShapeRetriever, self).__init__()
        self.pred_layer = nn.Sequential(
            nn.Linear(nz_feat, n_mean_shapes),
            nn.Softmax())
    
    def forward(self, feat):
        wt = self.pred_layer.forward(feat)
...
class ReconstructionNetwork(nn.Module):
    ...
    use_multitpl=False,n_templates=0,use_kp=False):
        super().__init__()
        ...
        self.use_multitpl = use_multitpl
        if self.use_multitpl:
            self.shape_wt = ShapeRetriever(nz_feat=256, n_mean_shapes=n_templates)
        self.use_kp = use_kp
        if self.use_kp:
            num_kps, num_verts  = 15, 482 # NOTE only for birds
            vert2kp_init = torch.Tensor(np.ones((num_kps, num_verts)) / float(num_verts))
            # vert2kp_init = torch.rand([num_kps,num_verts])
            ...
            # self.register_parameter(name='vert2kp',param=nn.Parameter(vert2kp_init))
    ...
    def forward(self, x):
        ...
        if self.use_kp:
            vert2kp = torch.nn.functional.softmax(self.vert2kp, dim=1) # [15, 482]
            other_outputs['vert2kp'] = vert2kp
        
        return tex, mesh_map, other_outputs
        #     # tex torch.Size([50, 3, 128, 128])  
        #     # mesh_map: torch.Size([50, 3, 128, 128])  
        #     return tex, mesh_map
