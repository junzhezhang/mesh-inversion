...
class GANLoss(nn.Module):
    ...
    def mean(self, x, mask=None, weight=None, get_average=True):
        if weight is None:
            weight = 1
    ...
    def loss(self, input, target_is_real, for_discriminator=True, mask=None, weight=None, get_average=True):
        if self.gan_mode == 'original':  # cross entropy loss
            ...
            if for_discriminator:
                if target_is_real:
                    ...
                    loss = -self.mean(minval, mask, weight, get_average=get_average)
                else:
                    minval = torch.min(-input - 1, self.get_zero_tensor(input))
                    loss = -self.mean(minval, mask, weight, get_average=get_average)
            else:
                assert target_is_real, "The generator's hinge loss must be aiming for real"
                loss = -self.mean(input, mask, weight, get_average=get_average)
            return loss
        else:
            # wgan
            if target_is_real:
    ...
    def __call__(self, input, target_is_real, for_discriminator=True, mask=None, weight=None,get_average=True):
        if isinstance(input, list):
            ...
            return self.loss(input, target_is_real, for_discriminator, mask,get_average=get_average)
